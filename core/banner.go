...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Jnr Page Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Jnr Nginx (") + handleClr.Sprintf("@jnrnginx") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
